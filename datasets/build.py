    ...
    *,
    ...
    Args:
        ...
        sampler (torch.utils.data.sampler.Sampler or None): a sampler that
            ...
            Default to :class:`InferenceSampler`, which splits the dataset across all workers. Sampler must be None
...
def build_detection_train_loader(
    ...
    Args:
        dataset (list or torch.utils.data.Dataset): a list of dataset dicts,
            or a map-style pytorch dataset. They can be obtained
            by using :func:`DatasetCatalog.get` or :func:`get_detection_dataset_dicts`.
...
def build_evaluator(cfg, dataset_name, output_folder=None):
    ...
    if evaluator_type in ["grounding_refcoco", "grounding_phrasecut", "grounding_spatial", "grounding_entity"]:
        evaluator_list.append(GroundingEvaluator(dataset_name, output_dir=str(cfg.get('MODEL', {}).get('BACKBONE', {}).get('NAME', 'not_found'))+"_"+str(cfg.get('MODEL', {}).get('BACKBONE', {}).get('RESNETS', {}).get('DEPTH', 'not_found'))))
    # Interactive
