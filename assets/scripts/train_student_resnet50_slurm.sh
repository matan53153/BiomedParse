#!/bin/bash
#SBATCH --job-name=biomedparse-train-resnet50 # Job name
...
#SBATCH --mem-per-cpu=16G            # Job memory request per CPU core (total 512GB)
...
#SBATCH --time=04:00:00              # Adjusted time limit hrs:min:sec
#SBATCH --output=slurm_train_resnet50_%j.log  # Standard output and error log (%j expands to jobId)
